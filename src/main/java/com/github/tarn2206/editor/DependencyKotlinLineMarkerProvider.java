...
public class DependencyKotlinLineMarkerProvider extends LineMarkerProviderDescriptor {
    ...
    public @Nullable LineMarkerInfo<?> getLineMarkerInfo(@NotNull PsiElement element) {
        ...
        if (!KotlinDslHelpers.isKotlinDslFile(file)) return null;

        var tpl = PsiTreeUtil.getParentOfType(element, KtStringTemplateExpression.class);
        if (tpl == null) return null;

        // Only attach the marker on the first leaf of the template — avoids duplicates.
        var firstLeaf = PsiTreeUtil.getDeepestFirst(tpl);
    ...
    private void handleClick(PsiElement element, Dependency dep, boolean hasUpdate) {
        ...
        if (hasUpdate && tpl != null && dep.getLatestVersion() != null) {
            if (KotlinDslHelpers.replaceVersionInTemplate(project, tpl, dep.getVersion(), dep.getLatestVersion())) {
